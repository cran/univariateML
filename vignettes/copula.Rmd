...
Then a natural model selection using is to
...
The nest step is to define a function that calculates the AIC for all our candidate models
over every margin.
...
This function returns a vector of AICs for each model where `ml***` didn't fail.
When it fails it is since the support of the data doesn't not match the density, 
for instance when `-1` is fed to the exponential density.
